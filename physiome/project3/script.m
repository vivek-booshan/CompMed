...
%%%% FUNCTIONS & DIFF SYSTEM %%%%%
U1 = @(t) 2*(floor(t) == 180);    % branchless definition
Ucho = @(t) 20*(floor(t) == 180); % branchless definition
f = @(t,z,i) [
...
%%%% ASSUME ONLY INSULIN BOLUS (UCHO = 0) %%%%
Ucho = @(t) 0;
f = @(t,z,i) [
    -ax*z(1) + ax*z(2);                            %eqn 17 (dX)
    -ax*z(2) + Kx(i)*ax*U1(t);                     %eqn 18 (dX1)
...
subtitle("[BG] with only insulin bolus");
xlabel('Time (hours)');
ylabel('BG concentration (mg/dl)');
grid; axis([0 930 45 90]);
legend('Kx = 20','Kx = 10','Location','NorthEast');
